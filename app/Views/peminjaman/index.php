      ...
      <?php if (!empty($peminjaman)) : ?>
        <?php foreach ($peminjaman as $row) : ?>
          <tr>
            ...
            <td>
              <span class="badge <?= $row['status'] === 'Dikembalikan' ? 'bg-success' : 'bg-warning text-dark' ?>">
                <?= esc($row['status']) ?>
              </span>
            </td>
            <td>
              <a href="<?= base_url('peminjaman/edit/' . $row['id']) ?>" class="btn btn-sm btn-warning">Edit</a>
              <form action="<?= base_url('peminjaman/delete/' . $row['id']) ?>" method="post" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                <?= csrf_field() ?>
